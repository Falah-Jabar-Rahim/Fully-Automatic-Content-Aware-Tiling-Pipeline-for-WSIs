...
from configs.inti_artifact import inti_model_artifact
...
if __name__ == "__main__":
    ...
    ## 1) initialize artifact detection model
    ...
    ## 2) initialize pen marker removal model
    ...
    for wsi_id, wsi_file in enumerate(wsi_files):
        ...
        # ---- Explicit memory cleanup per WSI ----
        del thumbnail
        del thumbnail_mask
        del thumbnail_roi
        del thumbnail_mask_3d
        del masked_rgb_image
        del masked_rgb_image_arr
        del wsi_seg_fill
        del wsi_clean_tiles_fill
        del wsi_seg_qt_fill
        del sel_tiles
        del tile_cords
        # DataLoader & its workers
        del data_loader
        # Encourage Python to release RAM
        gc.collect()
        # Encourage CUDA to release unused GPU memory
